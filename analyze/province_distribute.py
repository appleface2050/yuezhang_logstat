...
from model.stat import RunUserProvince,VisUserProvince
...
class Hive2db(object):
    ...
    def import_data(self, mode, time):
        ...
        run_user_data = self.org_service.get_run_user_province_stat(start,end)
        for data in run_user_data:
            try: 
                data = data.strip()
                i = data.split('\t')
                import_num = 0
                stat = RunUserProvince.new()
                stat.time = start.strftime('%Y-%m-%d')
                stat.version_name = i[0]
                stat.partner_id = i[1]
                stat.province = i[2]
                stat.uv = i[3]
                stat.save()
        ...
        visit_user_data = self.org_service.get_visit_user_province_stat(start,end)
        for data in visit_user_data:
            try:    
                data = data.strip()
                i = data.split('\t')
                stat = VisUserProvince.new()
                stat.time = start.strftime('%Y-%m-%d')
                stat.version_name = i[0]
                stat.partner_id = i[1]
                stat.province = i[2]
